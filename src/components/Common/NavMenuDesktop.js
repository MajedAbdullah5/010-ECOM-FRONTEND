...
class NavMenuDesktop extends Component {
    ...
    onRedirects() {
        if (this.state.readyToRedirect == true) {
            return <Redirect to={"/ProductSearch/"+this.state.searchItem}/>
        }
    }
    onLogout(){
                    ...
                    <Navbar fixed={"top"} bg="light" className="p-2">
                        <Row className="w-100">
                            <Col className="p-1" sm={4} md={4} sm={12} xs={12}>
                                <Link to={"/"}><a className="btn"><img className="nav-logo" src="https://demo.ecom.rabbil.com/static/media/BigExpress.432afd37.png" alt=""/></a></Link>
                                <Link to="/cart"><Button className="cart-btn"><i className="fa fa-shopping-cart"></i> {this.state.items}
                                     &nbsp;items</Button></Link>
                            </Col>
                            <Col className="p-1" sm={4} md={4} sm={12} xs={12}>
                                <div className="input-group w-100">
                                    <input onChange={this.onChanges} type="text" className="form-control"
                                           placeholder={this.state.placeholder}
                                           aria-label="Text input with segmented dropdown button"/>
                                    <button type="button" className="btn site-btn" onClick={this.onClicks}><i
                            ...
                            <Col className="p-1" sm={4} md={4} sm={12} xs={12}>
                                ...
                                <Link to="/notification" className="btn"><i className="fa h4  fa-bell"></i> <sup><span
                                    className="badge text-white bg-danger">4</span></sup></Link>
                                <Dropdown className="d-inline-flex float-right mr-5">
                                    <Dropdown.Toggle variant="" id="dropdown-basic">
                                        Account
                                    </Dropdown.Toggle>
                                    <Dropdown.Menu>
                                        <Dropdown.Item><Link to={"/"}>Favorite</Link></Dropdown.Item>
                                        <Dropdown.Item><Link to={"/"}>Wishlist</Link></Dropdown.Item>
                                        <Dropdown.Item><Link to={"/"}>Profile</Link></Dropdown.Item>
                                        <Dropdown.Item><Link onClick={this.onLogout}>Logout</Link></Dropdown.Item>
                                    </Dropdown.Menu>
                                </Dropdown>
                            </Col>
                        </Row>
                ...
                <Fragment>
                    <Navbar fixed={"top"} bg="light" className="p-2">
                        <Row className="w-100">
                            ...
                            <Col className="p-1" sm={4} md={4} sm={12} xs={12}>
                                <div className="input-group w-100">
                                    <input placeholder={this.state.placeholder} onChange={this.onChanges} type="text"
                                           className="form-control"
                                           aria-label="Text input with segmented dropdown button"/>
                                    <button type="button" onClick={this.onClicks} className="btn site-btn"><i
                                        className="fa fa-search"></i>
                                    </button>
                                </div>
