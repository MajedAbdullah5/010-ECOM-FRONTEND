    ...
    addToCart = (e) => {
        if (this.state.isLoggedIn != null) {
            ...
            if (isColor == "YES" && color.length == 0) {
                ...
                axios.post(ApiURL.AddToCart,formData,config).then(response => {
                    if (response.data === 1) {
    ...
    render() {
        ...
        if (!size.length == 0) {
            ...
            sizes = splt.map((size, i) => {
                return (
                    <span><input onChange={this.sizeOnChange} className="form-check-input" type="radio" name="sizes"
                                 id={size + i.toString()} value={size}/>{size}&emsp;&emsp;</span>
                );
            })
            displaysize = "";
        }
        ...
        if (!color.length == 0) {
            ...
            colors = colorSplit.map((color, i) => {
                return (
                    <span><input onChange={this.colorOnChange} className="form-check-input" type="radio"
                                 name="colors" id={color + i.toString()} value={color}/>{color}&emsp;&emsp;</span>
                );
            })
            displaycolor = "";
        }
                    ...
                    <Container fluid={true} className="TopSection onboardMargin mt-5 pt-5">
                        <Breadcrumb>
                            ...
                            <Breadcrumb.Item><Link
                                to={"/ProductListByCategory/" + category}>{category}</Link></Breadcrumb.Item>
                            <Breadcrumb.Item><Link
                                to={"/ProductListBySubCategory/" + category + "/" + subcategory}>{subcategory}</Link></Breadcrumb.Item>
                        </Breadcrumb>
                        <Row>
                                    ...
                                    <Col lg={6} md={6} sm={12} xs={12}>
                                        <Row>
                                            <Col sm={12} lg={12} md={12} xs={12} xl={12}>
                                                <InnerImageZoom className="main-image" src={this.state.image}
                                                                zoomSrc={main_image}
                                                                zoomScale={1.8} zoomType="hover"/>
                                            </Col>
                                            <Col sm={12} lg={12} md={12} xs={12} xl={12}>
                                                <Container className="my-3">
                                                    <Row>
                                                        <Col className="p-0 m-0" lg={3} ms={3} sm={6} xs={6} xl={3}>
                                                            <img onClick={this.setOnClick} className="w-100 h-75"
                                                                 height="" src={img1} alt=""/>
                                                        </Col>
                                                        <Col className="p-0 m-0" lg={3} ms={3} sm={6} xs={6} xl={3}>
                                                            <img onClick={this.setOnClick} className="w-100 h-75"
                                    ...
                                    <Col lg={6} md={6} sm={12} xs={12}>
                                        <Row>
                                            <Col xs={12} xl={12} lg={12} sm={12} md={12}>
                                                ...
                                                <span className="price-font font-weight-bolder"><strike
                                                    className="text-danger">Before: {price} TK</strike>&emsp;After: {discount_price} TK</span>
                                            </Col>
                                            <Col xs={12} xl={12} lg={12} sm={12} md={12}>
                                            ...
                                            <Col xs={12} xl={12} lg={12} sm={12} md={12}>
                                                ...
                                                <input onChange={this.quantityOnChange}
                                                       className="form-control text-center w-50" type="number" min="1"/>
                                            </Col>
                                            <Col className="my-3" xs={12} xl={12} lg={12} sm={12} md={12}>
                                                <Button onClick={this.addToCart} className="btn-danger mr-1"><i
                                                    className="fas fa-shopping-cart"></i> Add To Cart</Button>
                                                <Button className="mr-1"> <i className="fas fa-car"></i> Order
                                                    Now</Button>
                                                <Button className="mr-1"> <i
                                                    className="fas fa-heart"></i> Favourite</Button>
                                            </Col>
                                            <Col xs={12} xl={12} lg={12} sm={12} md={12}>
                                ...
                                <Row>
                                    <Col lg={6} md={6} sm={12} xs={12}>
                                        <Row>
                                            <Col sm={12} lg={12} md={12} xs={12} xl={12}>
                                                <InnerImageZoom className="main-image" src={this.state.image}
                                                                zoomSrc={this.state.image} zoomScale={1.8}
                                                                zoomType="hover"/>
                                            </Col>
                                            <Col sm={12} lg={12} md={12} xs={12} xl={12}>
                                                <Container className="my-3">
